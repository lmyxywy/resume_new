...
//iframe中src属性设置
document.getElementById("myiframe").src="http://139.215.208.13:8091/was5/web/search?channelid=236406&perpage=8"
}
...
//iframe中src属性设置
document.getElementById("myiframe").src="http://139.215.208.13:8091/was5/web/search?&channelid=236406&searchword=docreltime%3E%3D%27"+CurrentDate+"%27+and+docreltime%3C%3D%27"+CurrentDate+"%27&StringEncoding=UTF-8&perpage=8"
}
...
YUI({
	...
	modules:{
		'calendar-skin':{//默认皮肤
		    fullpath: 'css/calendar.css',///////////////使用需修改路径
			type:'css'
		},
		'calendar':{
		    fullpath: 'script/calendar.js',///////////////使用需修改路径
	...
	//静态日历,单击回调--1
    c = new Y.Calendar('video_date').on('select', function (d) {///////////////使用需修改id
		var day = new Date(Y.dump(d));
    var Year = 0;
    var Month = 0;
    var Day = 0;
    var CurrentDate = "";
...
if (Month >= 10 ){
    ...
    //静态日历,单击回调--2
    c = new Y.Calendar('import_video_date').on('select', function (d) {///////////////使用需修改id
        var day = new Date(Y.dump(d));
        var Year = 0;
        var Month = 0;
        var Day = 0;
